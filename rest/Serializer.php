...
class Serializer extends Component
{
    ...
    public function serialize($data)
    {
        ...
        } elseif ($data instanceof ResponeModel) {
            $res = array("status" => $data->status, "code" => $data->code, "message" => Yii::t('custom', $data->message), "data" => $data->data);
            if ($data->_meta !== null) {
    ...
    protected function serializeDataProvider($dataProvider)
    {
        ...
        if ($this->request->getIsHead()) {
            ...
            if ($pagination !== false) {
                return array_merge(array("status" => 200, "code" => 0, "message" => Yii::t('custom', "操作成功!")), $result, $this->serializePagination($pagination));
            } else {
                return $result;
            }
        }
    ...
    protected function serializeModel($model)
    {
        if ($this->request->getIsHead()) {
            ...
            return array("status" => 200, "code" => 0, "message" => Yii::t('custom', "操作成功!"), "data" => $model->toArray($fields, $expand));
        }
    }
